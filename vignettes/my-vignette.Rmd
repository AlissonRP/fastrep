...
```
# Overview
The objective of this package is to facilitate to produce reports using Rmarkdown, so the  functions are of   that type that you "you always have to use, but you always have to create"
...
```
The value of format will be automatically determined if the function is called within a knitr document, but in your section the default format is `html`, if you want to change just use the argument `format` which the possibles values are `html` or `latex`. If you want the code that generate the table you can use the argument  `code` 
```{r}
df |> 
...
```
If you want to generate a table using the output, just combine with `pdf1_tbl`
...
## `pdf1_freq_tbl2`
This function make cross tables, like all function from `mypdf1` you need to supply a `data.frame`, and is this case two variables.
```{r}
infert |> 
...
```
## `pdf1_na`
This function count the number os `NA` by variable in your `data.frame`
